...
function IsometricOffice({ lightOn = false, circuitActive = false }) {
  ...
  return (
    <svg viewBox="0 0 900 600" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style={{ position: "absolute", inset: 0 }}>
      ...
      {[1,2,3,4,5].map(i => (
        <line key={`wlv${i}`} x1={140 + 62*i} y1={60 + 50.4*i} x2={140 + 62*i} y2={312 - 50.4*(5-i) + 50.4*i - 50.4*i + 50.4*(i-1) + 50.4} stroke="#1e3a5f" strokeWidth="0.5" opacity="0.4" />
      ))}
      {[1,2,3].map(i => (
        <line key={`wlh${i}`} x1={140} y1={60 + (252) * i/4} x2={450} y2={60 + (80) * i/4} stroke="#1e3a5f" strokeWidth="0.5" opacity="0.3" />
      ))}
      <polygon points="450,60 760,60 760,312 450,140" fill="url(#wallRight)" stroke="#1e3a5f" strokeWidth="1.5" />
      {[1,2,3,4].map(i => (
        <line key={`wrv${i}`} x1={450 + 77.5*i} y1={60 + 20*i} x2={450 + 77.5*i} y2={140 + 43*i} stroke="#1e3a5f" strokeWidth="0.5" opacity="0.3" />
      ))}
      {[1,2,3].map(i => (
        <line key={`wrh${i}`} x1={450} y1={60 + 252*i/4} x2={760} y2={60 + 252*i/4} stroke="#1e3a5f" strokeWidth="0.5" opacity="0.25" />
      ))}
      ...
      {[[350,62],[450,62],[550,62],[400,62],[500,62]].map(([cx,cy],i) => (
        <g key={`clight${i}`}>
          <rect x={cx-12} y={cy-2} width={24} height={8} rx={2} fill="#0a1628" stroke="#1e3a5f" strokeWidth="1" />
          {glow > 0 && <ellipse cx={cx} cy={cy+18} rx={20+glow*30} ry={12+glow*18} fill={`rgba(255,228,100,${0.04+glow*0.12})`} />}
      ...
      <ellipse cx={560} cy={260} rx={80} ry={40} fill={`rgba(96,165,250,${0.03 + glow * 0.08})`} />
      ...
      {[0,1,2,3].map(i => (
        <line key={`shelf${i}`} x1={160} y1={155+37.5*i} x2={240} y2={110+37.5*i} stroke="#1e3a5f" strokeWidth="1" />
      ))}
      {[[168,148,8,28,"#3b82f6"],[178,145,6,27,"#ef4444"],[186,143,9,26,"#8b5cf6"],[197,140,7,25,"#10b981"],[168,186,8,28,"#f59e0b"],[178,183,10,27,"#6366f1"],[190,180,6,26,"#ec4899"],[168,222,8,27,"#14b8a6"],[178,219,7,26,"#f97316"],[187,217,9,25,"#3b82f6"]].map(([x,y,w,h,c],i) => (
        <polygon key={`book${i}`} points={`${x},${y} ${x+w},${y-w*0.5} ${x+w},${y-w*0.5+h} ${x},${y+h}`} fill={c} opacity="0.7" stroke={c} strokeWidth="0.5" />
      ))}
      ...
      <polygon points="406,184 494,138 494,244 406,290" fill={`rgba(59,130,246,${0.05+glow*0.12})`} />
      {glow > 0 && <>
        <polygon points="412,190 488,146 488,196 412,240" fill="rgba(6,20,48,0.9)" />
        {[0,1,2,3,4].map(i => (
          <line key={`sline${i}`} x1={415} y1={195+i*9} x2={415+(50+i*8)*0.82} y2={195+i*9-((50+i*8)*0.5)} stroke={["#3b82f6","#4ade80","#f59e0b","#8b5cf6","#60a5fa"][i]} strokeWidth={1.5} opacity="0.8" strokeLinecap="round" />
        ))}
        <polygon points="420,212 488,172 488,196 420,236" fill="rgba(15,30,60,0.8)" stroke="#1e3a5f" strokeWidth="0.5" />
        <polyline points="425,230 435,224 445,228 455,220 465,224 475,218 485,222" fill="none" stroke="#4ade80" strokeWidth="1" opacity="0.7" />
      </>}
      <circle cx={494} cy={246} r={2} fill={circuitActive ? "#4ade80" : "#1e3a5f"} style={circuitActive ? { filter: "url(#softGlow)" } : {}} />
      ...
      {[0,1,2,3].map(r => [0,1,2,3,4,5,6].map(c => (
        <rect key={`key${r}${c}`} x={383+c*11} y={282-c*5.5+r*7-r*3.5} width={8} height={5} rx={1} transform={`skewY(-26) skewX(0)`} fill="#0a1528" stroke="#1e3a5f" strokeWidth="0.4" opacity="0.8" />
      )))}

      <ellipse cx={478} cy={272} rx={12} ry={7} fill="#0e1e38" stroke="#1e3a5f" strokeWidth="1" transform="rotate(-26,478,272)" />
      <line x1={472} y1={269} x2={472} y2={275} stroke="#1e3a5f" strokeWidth="1" transform="rotate(-26,472,272)" />
      ...
      <path d="M 344,263 Q 352,264 352,270 Q 352,276 344,277" fill="none" stroke="#2a4a70" strokeWidth="1.5" />
      ...
      {[0,1,2,3].map(i => (<line key={`note${i}`} x1={358} y1={279+i*3.5} x2={395} y2={258+i*3.5} stroke="#1e3a5f" strokeWidth="0.6" />))}
      ...
      {[[-6,-16,12,-20],[6,-16,-12,-20],[0,-12,0,-22],[8,-14,-4,-24],[-8,-14,4,-24]].map(([dx1,dy1,dx2,dy2],i) => (
        <path key={`leaf${i}`} d={`M 595,266 Q ${595+dx1},${266+dy1} ${595+dx2},${266+dy2}`} fill="none" stroke="#22c55e" strokeWidth={i<2?"2.5":"2"} opacity="0.7" strokeLinecap="round" />
      ))}

      {[-16,0,16,-8,8].map((dx,i) => (
        <ellipse key={`wheel${i}`} cx={450+dx} cy={424+Math.abs(dx)*0.3} rx={4} ry={2.5} fill="#0a1628" stroke="#1e3654" strokeWidth="0.8" transform={`rotate(-20,${450+dx},${424+Math.abs(dx)*0.3})`} />
      ))}
      ...
      {[0,1,2,3,4,5,6,7].map(i => (
        <g key={`rack${i}`}>
          <polygon points={`663,${137+i*17.5} 717,${107+i*17.5} 717,${122+i*17.5} 663,${152+i*17.5}`}
            fill={i%3===0 ? "rgba(59,130,246,0.15)" : i%3===1 ? "rgba(74,222,128,0.1)" : "rgba(30,41,59,0.8)"} stroke="#1e3a5f" strokeWidth="0.8" />
          <circle cx={670} cy={144+i*17.5} r={2.5}
            fill={circuitActive && i%3!==2 ? (i%2===0 ? "#4ade80" : "#3b82f6") : "#1e3a5f"}
            style={circuitActive ? { filter: "url(#subtleGlow)" } : {}} />
          {circuitActive && i%3!==2 && <circle cx={680} cy={144+i*17.5} r={1.5} fill="#f59e0b" opacity="0.7" />}
        </g>
      ))}

      {circuitActive && ["M 490,244 Q 520,250 540,260 Q 560,270 570,280","M 335,280 Q 300,300 280,320 Q 260,340 250,360","M 600,280 Q 640,285 660,275"].map((d,i) => (
        <path key={`cable${i}`} d={d} fill="none" stroke={["#3b82f6","#4ade80","#f59e0b"][i]} strokeWidth={1.5} opacity="0.5" strokeDasharray="4,3" style={{ animation: "cableFlow 1.5s linear infinite" }} />
      ))}
      ...
      <text x={224} y={190} fontSize={7} fill="#60a5fa" opacity="0.8" transform="rotate(-27,224,190)">TABLERO</text>
      {[0,1,2,3,4,5].map(i => (
        <g key={`br${i}`}>
      ...
      <circle cx={258} cy={140} r={3} fill={circuitActive ? "#4ade80" : "#ef4444"}
        style={circuitActive ? { filter: "url(#softGlow)" } : {}} transform="rotate(-27,258,140)" />
    </svg>
  );
}
...
function useCircuit() {
  ...
  const analysis = useMemo(() => {
    // ─── 2. Si está apagado, retorna todo en cero ───
    if (!powerOn) {
      return {
        totalVoltage: 0, totalResistance: Infinity, current: 0, totalPower: 0,
        circuitClosed: false, compValues: {}, shortCircuit: false,
      };
    }
  ...
  // ─── 3. powerOn en dependencias ───
  }, [components, wires, powerOn]);
  ...
  // ─── 5. clearAll también apaga ───
  const clearAll = useCallback(() => {
    setComponents([]); setWires([]); setSelectedId(null); setPowerOn(false);
  }, []);
  ...
  // ─── 4. Exponer powerOn y setPowerOn ───
  return {
    ...
    deleteWire, toggleComp, updateComp, clearAll, getPorts, powerOn, setPowerOn,
  };
}
    ...
    <div style={{
      ...
      <aside style={{
        ...
        <div style={{ padding: "14px 14px 10px", borderBottom: "1px solid #0f2040" }}>
          ...
          <div style={{ fontSize: 7, color: "#1e3654", padding: "4px 0", lineHeight: 1.6 }}>
        ...
        <div style={{ padding: "12px 14px", borderBottom: "1px solid #0f2040" }}>
          <button
            ...
            style={{
              ...
              transition: "all 0.2s",
              background: powerOn ? "rgba(74,222,128,0.15)" : "rgba(239,68,68,0.12)",
              color: powerOn ? "#4ade80" : "#f87171",
              boxShadow: powerOn
                ? "0 0 12px rgba(74,222,128,0.2), inset 0 0 8px rgba(74,222,128,0.05)"
                : "0 0 8px rgba(239,68,68,0.1)",
              outline: `1px solid ${powerOn ? "rgba(74,222,128,0.35)" : "rgba(239,68,68,0.3)"}`,
            }}
          >
            <span style={{
              ...
              boxShadow: powerOn ? "0 0 6px #4ade80" : "0 0 4px #ef4444",
              animation: powerOn ? "blink 2s infinite" : "none",
            }} />
            {powerOn ? "ON" : "OFF"}
          </button>
          <div style={{ fontSize: 7, color: "#1e3654", textAlign: "center", marginTop: 5 }}>
            {powerOn ? "Sistema energizado" : "Sistema desenergizado"}
          </div>
        </div>
        ...
        {["Fuentes", "Cargas", "Control", "Protección"].map(cat => {
          ...
          return (
            <div key={cat}>
              <div style={{ padding: "10px 14px 4px", fontSize: 7.5, color: "#2a4a70", textTransform: "uppercase", letterSpacing: "0.18em", fontWeight: 700 }}>{cat}</div>
              {items.map(item => (
                <div key={item.type} className="pal-item" draggable
                  onDragStart={e => e.dataTransfer.setData("compType", item.type)}
                  style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 12px", cursor: "grab", margin: "1px 6px", borderRadius: 6, border: "1px solid transparent", transition: "all 0.15s", userSelect: "none" }}>
                  <div style={{ width: 38, height: 22, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(15,32,64,0.8)", border: "1px solid #0f2040", borderRadius: 4, flexShrink: 0, overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        ...
        <div ref={containerRef} style={{ position: "absolute", inset: 0 }}
          ...
          onMouseMove={handleMouseMove} onClick={handleCanvasClick}>
            ...
            {wires.map(w => {
              ...
              return (
                <g key={w.id}>
                  ...
                  <path d={d} fill="none" stroke={active ? "#b45309" : "#1e3a5f"} strokeWidth={active ? 4 : 2.5} strokeLinecap="round" />
          ...
          {components.map(comp => {
            ...
            return (
              <div key={comp.id} style={{ position: "absolute", left: comp.x - COMP_W / 2, top: comp.y - COMP_H / 2, width: COMP_W, height: COMP_H, zIndex: isSelected ? 50 : 10, cursor: pendingPort ? "crosshair" : "grab", animation: "fadeIn 0.2s ease" }}
                onMouseDown={e => handleCompMouseDown(comp.id, e)}
                onDoubleClick={e => { e.stopPropagation(); if (comp.type === "switch" || comp.type === "breaker") toggleComp(comp.id); }}>
                {isSelected && <div style={{ position: "absolute", inset: -5, borderRadius: 8, border: "1.5px solid #3b82f6", boxShadow: "0 0 0 3px rgba(59,130,246,0.2)", pointerEvents: "none" }} />}
                {active && <div style={{ position: "absolute", inset: -10, borderRadius: 12, background: "radial-gradient(ellipse, rgba(74,222,128,0.1) 0%, transparent 70%)", pointerEvents: "none" }} />}
                <div style={{ position: "relative" }}><CompSVG comp={comp} active={active} /></div>
                <div style={{ position: "absolute", top: COMP_H + 3, left: "50%", transform: "translateX(-50%)", fontSize: 8, color: "#3a5a80", whiteSpace: "nowrap", pointerEvents: "none", background: "rgba(3,11,31,0.8)", padding: "1px 4px", borderRadius: 3 }}>{comp.label}</div>
                {active && compData && (
                  <div style={{ position: "absolute", top: -30, left: "50%", transform: "translateX(-50%)", background: "rgba(10,20,40,0.95)", border: "1px solid #92400e", color: "#fde68a", fontSize: 7.5, padding: "2px 7px", borderRadius: 4, whiteSpace: "nowrap", pointerEvents: "none" }}>
                    {compData.v.toFixed(1)}V · {compData.i.toFixed(2)}A
                  </div>
                )}
                {isSelected && (
                  <button style={{ position: "absolute", top: -12, right: -12, width: 18, height: 18, borderRadius: "50%", background: "#ef4444", color: "white", border: "none", cursor: "pointer", fontSize: 12, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}
                    onMouseDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); deleteComponent(comp.id); }}>×</button>
                )}
                {getPorts().map(port => {
                  const isPending = pendingPort?.compId === comp.id && pendingPort?.portId === port.id;
                  const hasWire = wires.some(w => (w.fromCompId === comp.id && w.fromPortId === port.id) || (w.toCompId === comp.id && w.toPortId === port.id));
                  return (
                    <div key={port.id} className="port-dot" style={{ position: "absolute", left: COMP_W / 2 + port.dx - PORT_R, top: COMP_H / 2 + port.dy - PORT_R, width: PORT_R * 2, height: PORT_R * 2, borderRadius: "50%", background: isPending ? "#3b82f6" : hasWire ? (active ? "#f59e0b" : "#1e3a5f") : "rgba(59,130,246,0.1)", border: `2px solid ${isPending ? "#60a5fa" : hasWire ? (active ? "#f59e0b" : "#2a4a70") : "#3b82f6"}`, cursor: "crosshair", zIndex: 20, transition: "all 0.15s", boxShadow: isPending ? "0 0 8px rgba(59,130,246,0.8)" : "none" }}
          ...
          {components.length === 0 && (
            <div style={{ position: "absolute", bottom: 24, left: "50%", transform: "translateX(-50%)", pointerEvents: "none", background: "rgba(3,11,31,0.85)", border: "1px solid #0f2040", borderRadius: 10, padding: "10px 20px", color: "#2a4a70", fontSize: 10, textAlign: "center", lineHeight: 1.8 }}>
              <span style={{ color: "#3b82f6" }}>⚡</span> Arrastra componentes al lienzo isométrico
            </div>
          )}
          {pendingPort && (
            <div style={{ position: "absolute", top: 14, left: "50%", transform: "translateX(-50%)", background: "rgba(59,130,246,0.12)", border: "1px solid #3b82f6", color: "#60a5fa", fontSize: 10, padding: "5px 16px", borderRadius: 20, pointerEvents: "none" }}>
        ...
        <div style={{ position: "absolute", top: 14, right: 14, zIndex: 100 }}>
          <div style={{ background: analysis.circuitClosed ? "rgba(74,222,128,0.1)" : "rgba(15,32,64,0.8)", border: `1px solid ${analysis.circuitClosed ? "rgba(74,222,128,0.35)" : "#0f2040"}`, color: analysis.circuitClosed ? "#4ade80" : "#2a4a70", fontSize: 9, padding: "4px 12px", borderRadius: 20, display: "flex", alignItems: "center", gap: 6, letterSpacing: "0.12em" }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: analysis.circuitClosed ? "#4ade80" : "#1e3a5f", display: "inline-block", ...(analysis.circuitClosed ? { animation: "blink 1.2s infinite" } : {}) }} />
            {analysis.circuitClosed ? "CIRCUITO CERRADO" : "CIRCUITO ABIERTO"}
          </div>
          {analysis.shortCircuit && (
            <div style={{ marginTop: 6, background: "rgba(239,68,68,0.15)", border: "1px solid #ef4444", color: "#ef4444", fontSize: 9, padding: "4px 12px", borderRadius: 20, textAlign: "center" }}>
              ⚠ CORTOCIRCUITO
            </div>
          )}
        </div>
      ...
      {/* ══ RIGHT PANEL ══ */}
      <aside style={{ width: 220, background: "#020916", borderLeft: "1px solid #0f2040", display: "flex", flexDirection: "column", overflowY: "auto", flexShrink: 0 }}>
        <div style={{ padding: "14px", borderBottom: "1px solid #0f2040" }}>
          <div style={{ fontSize: 8, color: "#2a4a70", letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 12 }}>Análisis de Circuito</div>
          {[
            ...
            <div key={row.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0", borderBottom: "1px solid #080f20" }}>
              <span style={{ fontSize: 9.5, color: "#2a4a70" }}>{row.label}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: row.color }}>
                {row.value}<span style={{ fontSize: 8, marginLeft: 2, opacity: 0.5 }}>{row.unit}</span>
              </span>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: "10px 14px", borderBottom: "1px solid #0f2040" }}>
          <div style={{ fontSize: 7.5, color: "#2a4a70", letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 6 }}>Fórmulas</div>
          {["V = I · R", "P = V · I", "R_s = ΣR", "1/R_p = Σ(1/R)"].map(f => (
            <div key={f} style={{ fontSize: 9.5, color: "#1e3654", padding: "2px 0", fontFamily: "monospace" }}>{f}</div>
          ))}
        </div>

        {selectedComp ? (
          <div style={{ padding: "12px 14px", animation: "fadeIn 0.2s ease" }}>
            <div style={{ fontSize: 8, color: "#2a4a70", letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 10 }}>Propiedades</div>
            <div style={{ fontSize: 8, color: "#60a5fa", background: "rgba(59,130,246,0.08)", border: "1px solid rgba(59,130,246,0.2)", padding: "3px 8px", borderRadius: 4, display: "inline-block", marginBottom: 10 }}>{selectedComp.type.toUpperCase()}</div>
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 8, color: "#1e3654", marginBottom: 3 }}>Etiqueta</div>
              <input value={selectedComp.label} onChange={e => updateComp(selectedComp.id, { label: e.target.value })}
                style={{ width: "100%", background: "#040d1e", border: "1px solid #0f2040", borderRadius: 4, color: "#7aa8d8", fontSize: 10, padding: "4px 7px", outline: "none", boxSizing: "border-box" }} />
            </div>
            {(selectedComp.type === "battery" || selectedComp.type === "outlet") && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 8, color: "#1e3654", marginBottom: 3 }}>Voltaje (V)</div>
                <input type="number" value={selectedComp.voltage ?? ""} onChange={e => updateComp(selectedComp.id, { voltage: +e.target.value })}
            ...
            {selectedComp.type === "resistor" && (
              <div style={{ marginBottom: 10 }}>
                ...
                <input type="number" value={selectedComp.resistance ?? ""} onChange={e => updateComp(selectedComp.id, { resistance: +e.target.value })}
            ...
            {selectedComp.type === "luminaire" && (
              <div style={{ marginBottom: 10 }}>
                ...
                <input type="number" value={selectedComp.power ?? ""} onChange={e => updateComp(selectedComp.id, { power: +e.target.value })}
            ...
            {analysis.compValues[selectedComp.id] && analysis.circuitClosed && (
              <div style={{ marginTop: 10, padding: 10, background: "rgba(251,191,36,0.04)", border: "1px solid #78350f", borderRadius: 6 }}>
                ...
                {[["V caída", `${analysis.compValues[selectedComp.id].v.toFixed(2)} V`],["Corriente", `${analysis.compValues[selectedComp.id].i.toFixed(3)} A`],["Potencia", `${analysis.compValues[selectedComp.id].p.toFixed(2)} W`]].map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 9.5, padding: "2px 0", color: "#fde68a" }}>
                    <span style={{ color: "#78350f" }}>{k}</span><span>{v}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{ marginTop: "auto", padding: "10px 14px", borderTop: "1px solid #0f2040" }}>
          <button onClick={clearAll}
            style={{ width: "100%", background: "transparent", border: "1px solid #0f2040", color: "#1e3654", fontSize: 9, fontFamily: "monospace", padding: "7px", borderRadius: 5, cursor: "pointer", letterSpacing: "0.1em", transition: "all 0.15s" }}
